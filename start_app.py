...
def main():
    ...
    # 1. Sync Latest Legal Knowledge (Dictionary & Case Stories)
    print("🤖 [LEGAL SYNC] Updating Law Scholars' Hub via AI...")
    try:
        subprocess.run([flask_python, "legal_sync.py"], cwd=root_dir, check=True)
    except Exception as e:
        print(f"⚠️  Legal Sync Warning: {e}")
